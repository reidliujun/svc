...
static void CSSgenbits(unsigned char *output, int len,unsigned char const *s)
{
	...
	do {
		...
		for (bit = 0, val = 0; bit < 8; ++bit) {
			...
			o_lfsr0 = (unsigned char)(((lfsr0 >> 24) ^ (lfsr0 >> 21) ^ (lfsr0 >> 20) ^ (lfsr0 >> 12)) & 1);
			  lfsr0 = (lfsr0 << 1) | o_lfsr0;

			o_lfsr1 = (unsigned char)(((lfsr1 >> 16) ^ (lfsr1 >> 2)) & 1);
			  lfsr1 = (lfsr1 << 1) | o_lfsr1;

#define  BIT0(x) ((x) & 1)
#define  BIT1(x) (((x) >> 1) & 1)

			combined = (unsigned char)(!o_lfsr1 + b1_combined + !o_lfsr0);
			b1_combined = (unsigned char)(BIT1(combined));
...
static const unsigned char CSSvarients[] = {
	0xB7, 0x74, 0x85, 0xD0, 0xCC, 0xDB, 0xCA, 0x73,
	0x03, 0xFE, 0x31, 0x03, 0x52, 0xE0, 0xB7, 0x42,
	0x63, 0x16, 0xF2, 0x2A, 0x79, 0x52, 0xFF, 0x1B,
	0x7A, 0x11, 0xCA, 0x1A, 0x9B, 0x40, 0xAD, 0x01};

static const unsigned char CSSsecret[] = {0x55, 0xD6, 0xC4, 0xC5, 0x28};

static const unsigned char CSStable0[] = {
...
static const unsigned char CSStable1[] = {
...
static const unsigned char CSStable2[] = {
...
static const unsigned char CSStable3[] = {
...
static void CSSengine(int varient, unsigned char const *input,unsigned char *output)
{
	 ...
	 * algorithm.
	 */
	cse = (unsigned char)(CSSvarients[varient] ^ CSStable2[varient]);
	...
	for (i = 5, term = 0; --i >= 0; term = input[i]) {
		index = (unsigned char)(bits[25 + i] ^ input[i]);
		index = (unsigned char)(CSStable1[index] ^ ~CSStable2[index] ^ cse);

		temp1[i] = (unsigned char)(CSStable2[index] ^ CSStable3[index] ^ term);
	}
	temp1[4] ^= temp1[0];

	for (i = 5, term = 0; --i >= 0; term = temp1[i]) {
		index = (unsigned char)(bits[20 + i] ^ temp1[i]);
		index = (unsigned char)(CSStable1[index] ^ ~CSStable2[index] ^ cse);

		temp2[i] = (unsigned char)(CSStable2[index] ^ CSStable3[index] ^ term);
	}
	temp2[4] ^= temp2[0];

	for (i = 5, term = 0; --i >= 0; term = temp2[i]) {
		index = (unsigned char)(bits[15 + i] ^ temp2[i]);
		index = (unsigned char)(CSStable1[index] ^ ~CSStable2[index] ^ cse);
		index = (unsigned char)(CSStable2[index] ^ CSStable3[index] ^ term);

		temp1[i] = (unsigned char)(CSStable0[index] ^ CSStable2[index]);
	}
	temp1[4] ^= temp1[0];

	for (i = 5, term = 0; --i >= 0; term = temp1[i]) {
		index = (unsigned char)(bits[10 + i] ^ temp1[i]);
		index = (unsigned char)(CSStable1[index] ^ ~CSStable2[index] ^ cse);

		index = (unsigned char)(CSStable2[index] ^ CSStable3[index] ^ term);

		temp2[i] = (unsigned char)(CSStable0[index] ^ CSStable2[index]);
	}
	temp2[4] ^= temp2[0];

	for (i = 5, term = 0; --i >= 0; term = temp2[i]) {
		index = (unsigned char)(bits[5 + i] ^ temp2[i]);
		index = (unsigned char)(CSStable1[index] ^ ~CSStable2[index] ^ cse);
	...
	for (i = 5, term = 0; --i >= 0; term = temp1[i]) {
		index = (unsigned char)(bits[i] ^ temp1[i]);
		index = (unsigned char)(CSStable1[index] ^ ~CSStable2[index] ^ cse);

		output[i] = (unsigned char)(CSStable2[index] ^ CSStable3[index] ^ term);
	}
}
