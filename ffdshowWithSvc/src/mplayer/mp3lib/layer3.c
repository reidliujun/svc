...
static void init_layer3(int down_sample_sblimit)
{
  ...
  {
    if(_has_mmx)
      gainpow2[i+256] = (real)(16384.0 * pow((double)2.0,-0.25 * (double) (i+210) ));
    else
...
#ifdef NEW_DCT9
  cos9[0] = (real)cos(1.0*M_PI/9.0);
  cos9[1] = (real)cos(5.0*M_PI/9.0);
  cos9[2] = (real)cos(7.0*M_PI/9.0);
  cos18[0] = (real)cos(1.0*M_PI/18.0);
  cos18[1] = (real)cos(11.0*M_PI/18.0);
  cos18[2] = (real)cos(13.0*M_PI/18.0);
#endif
 ...
 int ms_stereo,int sfreq,int single,int lsf)
{
   ...
   si->main_data_begin = getbits(ctx,tab[1]);
   if (stereo == 1)
     si->private_bits = getbits_fast(ctx,tab[2]);
   else 
     si->private_bits = getbits_fast(ctx,tab[3]);

   if(!lsf) {
     for (ch=0; ch<stereo; ch++) {
         si->ch[ch].gr[0].scfsi = -1;
         si->ch[ch].gr[1].scfsi = getbits_fast(ctx,4);
     }
   }
   ...
   for (gr=0; gr<tab[0]; gr++) {
     for (ch=0; ch<stereo; ch++) {
       ...
       gr_info->part2_3_length = getbits(ctx,12);
       gr_info->big_values = getbits(ctx,9);
       if(gr_info->big_values > 288) {
            fprintf(stderr,"big_values too large!\n");
          gr_info->big_values = 288;
       }
       gr_info->pow2gain = gainpow2+256 - getbits_fast(ctx,8) + powdiff;
       if(ms_stereo)
         gr_info->pow2gain += 2;
       gr_info->scalefac_compress = getbits(ctx,tab[4]);

       if(get1bit(ctx)) { /* window switch flag  */
         int i;
#ifdef L3_DEBUG
if(2*gr_info->big_values > bandInfo[sfreq].shortIdx[12])
  fprintf(stderr,"L3: BigValues too large, doesn't make sense %d %d\n",2*gr_info->big_values,bandInfo[sfreq].shortIdx[12]);
         ...
         gr_info->block_type       = getbits_fast(ctx,2);
         gr_info->mixed_block_flag = get1bit(ctx);
         gr_info->table_select[0]  = getbits_fast(ctx,5);
         gr_info->table_select[1]  = getbits_fast(ctx,5);
         /*
         ...
         for(i=0;i<3;i++)
           gr_info->full_gain[i] = gr_info->pow2gain + (getbits_fast(ctx,3)<<3);
...
if(2*gr_info->big_values > bandInfo[sfreq].longIdx[21])
  fprintf(stderr,"L3: BigValues too large, doesn't make sense %d %d\n",2*gr_info->big_values,bandInfo[sfreq].longIdx[21]);
#endif
         for (i=0; i<3; i++)
           gr_info->table_select[i] = getbits_fast(ctx,5);
         r0c = getbits_fast(ctx,4);
         r1c = getbits_fast(ctx,3);
       ...
       if(!lsf)
         gr_info->preflag = get1bit(ctx);
       gr_info->scalefac_scale = get1bit(ctx);
       gr_info->count1table_select = get1bit(ctx);
     }
   }
 ...
 * read scalefactors
 */
static int III_get_scale_factors_1(mp3lib_ctx *ctx,int *scf,struct gr_info_s *gr_info)
{
