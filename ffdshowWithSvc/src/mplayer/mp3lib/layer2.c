...
static void II_step_one(mp3lib_ctx *ctx,unsigned int *bit_alloc,int *scale,struct frame *fr)
{
    ...
    if(stereo)
    {
      ...
      {
        *bita++ = (char) getbits(ctx,step=alloc1->bits);
        *bita++ = (char) getbits(ctx,step);
      }
      for (i=sblimit-jsbound;i>0;i--,alloc1+=(1<<step))
      {
        bita[0] = (char) getbits(ctx,step=alloc1->bits);
        bita[1] = bita[0];
        bita+=2;
      }
      bita = bit_alloc;
...
static void II_step_two(mp3lib_ctx *ctx,unsigned int *bit_alloc,real fraction[2][4][SBLIMIT],int *scale,struct frame *fr,int x1)
{
    int i,j,k,ba;
      ...
      for (j=0;j<stereo;j++)
      {
        if ( (ba=*bita++) ) 
        {
          ...
          else 
          {
            ...
            idx = (unsigned int) getbits(ctx,k);
    ...
    for (i=jsbound;i<sblimit;i++,alloc1+=(1<<step))
    {
      ...
      if ( (ba=*bita++) )
      {
        ...
        if( (d1=alloc2->d) < 0)
        {
          ...
          fraction[1][0][i] = (fraction[0][0][i] = (real) ((int)getbits(ctx,k) + d1) ) * cm;
          fraction[1][1][i] = (fraction[0][1][i] = (real) ((int)getbits(ctx,k) + d1) ) * cm;
          fraction[1][2][i] = (fraction[0][2][i] = (real) ((int)getbits(ctx,k) + d1) ) * cm;
          cm=muls[k][scale[x1]];
...
{
  static const int translate[9][2][16] =
...
static int do_layer2(mp3lib_ctx *ctx,int outmode)
{
  ...
  outmode=outmode; //to kill "unused parameter" warning
  ...
  for (i=0;i<SCALE_BLOCK;i++) 
  {
    ...
    for (j=0;j<3;j++) 
    {
      if(single >= 0)
      {
        clip += (fr->synth_mono) (ctx,fraction[single][j],ctx->pcm_sample,&ctx->pcm_point);
      }
      else {
          int p1 = ctx->pcm_point;
          clip += (fr->synth) (ctx,fraction[0][j],0,ctx->pcm_sample,&p1);
          clip += (fr->synth) (ctx,fraction[1][j],1,ctx->pcm_sample,&ctx->pcm_point);
      }
